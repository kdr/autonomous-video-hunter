...
video_understander = VideoUnderstander(
    api_key=os.environ.get("CLOUDGLUE_API_KEY"))


def get_video_context(video_path: str,
                      include_local_frames: bool = False) -> Dict[str, Any]:
    """Get comprehensive context and summary for a video from the context store"""
    context = context_store.get_current_context(
        video_path, include_local_frames=include_local_frames)
    if context is None:
        return {
            "error": "Video not found in context database",
...
def search_video_collection(
    ...
    collection = context_store.get_collection_context(
        start=start, limit=limit, include_local_frames=include_local_frames)
...
def detect_faces_in_video(video_path: str,
                          ...
                          topk: int = 5,
                          crop_faces: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        if not local_frames:
            return {
                "error": "No frames available for face detection",
                "video_path": video_path
            }

        # Initialize face matcher
        face_matcher = FaceMatcher(reference_images=reference_images,
        ...
        for frame_info in local_frames:
            ...
            frame_path = frame_info.get("thumbnail_path") if isinstance(
                frame_info, dict) else frame_info
            if frame_path and Path(frame_path).exists():
                matches = face_matcher.matches(frame_path,
                                               topk=topk,
                                               crop=crop_faces)
                if matches:
                    results.append({
                        ...
                        frame_path,
                        "timestamp":
                        frame_info.get("timestamp") if isinstance(
                            frame_info, dict) else None,
                        "matches":
                        matches
                    })
...
def detect_objects_in_video(video_path: str,
                            target_objects: List[str],
                            topk: int = 3,
                            crop_objects: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        if not local_frames:
            return {
                "error": "No frames available for object detection",
                "video_path": video_path
            }
            ...
            if frame_path and Path(frame_path).exists():
                ...
                for obj_class, detections_list in detections.items():
                    high_conf_detections = [
                        d for d in detections_list if d.get("score", 0) > 0.3
                    ]
...
def match_images_in_video(video_path: str,
                          reference_images: List[str],
                          reference_labels: Optional[List[str]] = None,
                          min_inliers: int = 15,
                          min_inlier_ratio: float = 0.4) -> Dict[str, Any]:
    ...
    try:
        ...
        if not local_frames:
            ...
            return {
                "error": "No frames available for image matching",
                "video_path": video_path
            }
        ...
        print(
            f"  🔧 Creating ImageRansacDBMatcher with {len(existing_refs)} reference images..."
        )
        matcher = ImageRansacDBMatcher(image_paths=existing_refs,
        ...
        for i, frame_info in enumerate(local_frames):
            ...
            frame_path = frame_info.get("thumbnail_path") if isinstance(
                frame_info, dict) else frame_info
            timestamp = frame_info.get("timestamp") if isinstance(
                frame_info, dict) else None
            ...
            if matches:
                ...
                for j, match in enumerate(matches):
                    ...
                    print(
                        f"        Match {j+1}: {match.get('label')} - {details.get('num_inliers', 0)} inliers, ratio: {details.get('inlier_ratio', 0):.3f}"
                    )
...
def extract_temporal_info(
    video_path: str,
    info_type: Literal["time_of_day", "weather", "season",
                       "location_clues"] = "time_of_day"
) -> Dict[str, Any]:
    ...
    try:
        ...
        temporal_clues = {
            "is_outdoors": summary.get("is_outdoors", False),
            "has_natural_lighting":
            None,  # Could be enhanced with image analysis
            "weather_indicators": [],
            "time_indicators": [],
            "location_clues": []
        }
        ...
        if description:
            ...
            temporal_clues["time_indicators"] = [
                kw for kw in time_keywords if kw in desc_lower
            ]

            # Weather indicators
            weather_keywords = [
                "sunny", "cloudy", "rainy", "snowy", "foggy", "clear",
                "overcast"
            ]
            temporal_clues["weather_indicators"] = [
                kw for kw in weather_keywords if kw in desc_lower
            ]

            # Location clues
            location_keywords = [
                "urban", "rural", "city", "countryside", "beach", "mountain",
                "forest", "indoor", "outdoor"
            ]
            temporal_clues["location_clues"] = [
                kw for kw in location_keywords if kw in desc_lower
            ]
...
def compare_videos(
    ...
    comparison_type: Literal["visual_similarity", "content_similarity",
                             "temporal_sequence"] = "content_similarity"
) -> Dict[str, Any]:
    ...
    try:
        ...
        for i in range(len(video_contexts)):
            for j in range(i + 1, len(video_contexts)):
                ...
                similarity_metrics = {
                    "has_face_both":
                    vid1.get("summary", {}).get("has_face")
                    and vid2.get("summary", {}).get("has_face"),
                    "is_outdoors_both":
                    vid1.get("summary", {}).get("is_outdoors")
                    and vid2.get("summary", {}).get("is_outdoors"),
                    "has_speech_both":
                    vid1.get("summary", {}).get("has_speech")
                    and vid2.get("summary", {}).get("has_speech"),
                    "has_logo_both":
                    vid1.get("summary", {}).get("has_logo")
                    and vid2.get("summary", {}).get("has_logo")
                }

                # Calculate simple similarity score
                similarity_score = sum(
                    similarity_metrics.values()) / len(similarity_metrics)
...
def add_video_memory(video_path: str, memory_name: str,
...
def cross_reference_findings(entity_type: Literal["person", "object",
                                                  "location", "logo"],
                             entity_identifier: str,
                             search_all_videos: bool = True) -> Dict[str, Any]:
    """Cross-reference a person, object, or location across multiple videos"""
    try:
        if search_all_videos:
            collection = context_store.get_collection_context(start=0,
                                                              limit=1000)
            videos_to_search = [
                v["video_path"] for v in collection["video_contexts"]
            ]
        else:
            videos_to_search = []
